...
import {
  ArrowLeft, Filter, Download, BarChart3, Users, CheckCircle2, XCircle,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useAuth, apiFetch, FIELD_CONFIG, getGrade, handlePrintPDF, KopSurat, type View } from "@/lib/app-shared";
...
const IYA_COLOR = "#ef4444";   // red-500 (IYA = masalah/problem → merah)
const TIDAK_COLOR = "#10b981"; // emerald-500 (TIDAK = tidak bermasalah → hijau)
...
export default function AdminMonitoring({ onNavigate }: { onNavigate: (view: View) => void }) {
  ...
  const buildStudentData = (): StudentMonitoring[] => {
    ...
    const userMap = new Map<string, { id: string; name: string; grade: number; whatsapp: string | null; iyaCount: number; tidakCount: number; total: number }>();
  ...
  const students = buildStudentData();
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-500">Belum ada data siswa yang mengerjakan survey</p>
            </CardContent>
          </Card>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-3xl font-bold text-red-600">{totalIYA}</p>
                  <p className="text-sm text-red-500">{overallIyaPct}%</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                ...
                <div style={{ width: "100%", minHeight: students.length * 48 + 60 }}>
                  <ResponsiveContainer width="100%" height={students.length * 48 + 60}>
                    ...
                    >
                      ...
                      <Bar dataKey="TIDAK" stackId="a" fill={TIDAK_COLOR} radius={[0, 0, 0, 0]}>
                        {chartData.map((entry, index) => (
                          <Cell key={`tidak-${index}`} fill={TIDAK_COLOR} />
                        ))}
                      </Bar>
                      <Bar dataKey="IYA" stackId="a" fill={IYA_COLOR} radius={[0, 4, 4, 0]}>
                        {chartData.map((entry, index) => (
                        ...
                        return (
                          <tr key={s.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-3 text-center text-sm">
                              {s.whatsapp ? (
                                <a
                                  href={`https://wa.me/${s.whatsapp.startsWith('0') ? '62' + s.whatsapp.slice(1) : s.whatsapp.startsWith('62') ? s.whatsapp : '62' + s.whatsapp}`}
                                  target="_blank"
                                ...
                                >
                                  ...
                                  {s.whatsapp.startsWith('0') ? s.whatsapp : s.whatsapp.startsWith('62') ? '0' + s.whatsapp.slice(2) : s.whatsapp}
                                </a>
                              ) : (
                            ...
                            <td className="p-3 text-center">
                              <Badge className={`${gInfo.bgColor} ${gInfo.color}`}>
