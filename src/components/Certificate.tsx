...
import { useAuth, apiFetch, FIELD_CONFIG, type View, type ResponseData, handlePrintPDF } from "@/lib/app-shared";
import {
  getGradeInfo,
...
export default function Certificate({ onNavigate, studentId }: CertificateProps) {
  ...
  const [studentProfile, setStudentProfile] = useState<{ name: string; grade: number; jenisKelamin?: string | null; email: string; whatsapp?: string | null; createdAt?: string } | null>(null);
  ...
  // Check if student qualifies for certificate: 100% TIDAK across ALL 4 fields = no problems
  const allFieldsCompleted = ["PRIBADI", "SOSIAL", "BELAJAR", "KARIR"].every((f) => fieldAnalysis[f]);
  const allFieldsPerfect = allFieldsCompleted && Object.values(fieldAnalysis).every((d) => d.percentage === 100);
  const qualifiesForCertificate = allFieldsPerfect && overallData?.percentage === 100;
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <Card className="p-8 text-center">
          ...
          <p className="text-gray-500 mb-4">
            Sertifikat diberikan kepada siswa yang mencapai <strong>100% TIDAK</strong> (tidak ada masalah) di seluruh bidang assessment.
          </p>
          {!allFieldsCompleted && (
          ...
          {allFieldsCompleted && !allFieldsPerfect && (
            <div className="mt-4">
              <p className="text-sm text-gray-600 mb-3">Pencapaian siswa saat ini:</p>
                ...
                {(["PRIBADI", "SOSIAL", "BELAJAR", "KARIR"] as FieldName[]).map((field) => {
                  ...
                  return (
                    <div key={field} className="text-center p-3 rounded-lg bg-gray-50 border">
                      ...
                      <p className={`text-lg font-bold ${pct === 100 ? "text-emerald-600" : "text-gray-400"}`}>
                        {pct}%
                      </p>
                    </div>
                  );
          ...
          <div className="relative border-[2px] border-amber-500/50 px-3 py-2 sm:px-5 sm:py-3 bg-gradient-to-br from-amber-50/50 via-white to-emerald-50/50">
            ...
            <div className="text-center mb-2">
              <p className="text-[9px] sm:text-xs text-gray-700 leading-relaxed max-w-xl mx-auto">
                Atas pencapaian <strong className="text-emerald-700">100% TIDAK (Tidak Ada Masalah)</strong> pada
                seluruh bidang assessment <strong>Daftar Cek Masalah (DCM)</strong> Bimbingan Konseling,
                yang menunjukkan kondisi siswa dalam keadaan <strong className="text-emerald-700">Baik</strong> di semua aspek.
              </p>
            </div>
              ...
              <table className="w-full max-w-xs mx-auto border-collapse">
                ...
                <tbody>
                  {(["PRIBADI", "SOSIAL", "BELAJAR", "KARIR"] as FieldName[]).map((field, idx) => {
                    ...
                    return (
                      <tr key={field} className={idx % 2 === 0 ? "bg-emerald-50/50" : "bg-white"}>
                        ...
                        <td className="border border-gray-200 px-2 py-0.5 text-center">
                          <span className="inline-flex items-center gap-0.5 bg-emerald-100 text-emerald-700 text-[8px] sm:text-[10px] font-bold px-1.5 py-0 rounded-full">
                            <CheckCircle2 className="h-2 w-2" /> A
                          </span>
                        </td>
                      </tr>
                    );
                  ...
                  <tr className="bg-gradient-to-r from-amber-50 to-amber-100/80 font-semibold">
                    ...
                    <td className="border border-gray-200 px-2 py-0.5 text-center">
                      <span className="text-[9px] sm:text-xs font-bold text-amber-700">100% TIDAK</span>
                    </td>
                    <td className="border border-gray-200 px-2 py-0.5 text-center">
