...
const PIE_COLORS: Record<string, string> = {
  IYA: "#ef4444",     // IYA = problem (red/merah)
  TIDAK: "#10b981",   // TIDAK = no problem (emerald/hijau)
};

const FIELD_PIE_COLORS: Record<string, { iya: string; tidak: string }> = {
  PRIBADI: { iya: "#ef4444", tidak: "#10b981" },   // red for IYA, emerald for TIDAK
  SOSIAL: { iya: "#ef4444", tidak: "#10b981" },
  BELAJAR: { iya: "#ef4444", tidak: "#10b981" },
  KARIR: { iya: "#ef4444", tidak: "#10b981" },
};
...
export default function ResultsPage({ onNavigate, surveyIdFilter }: { onNavigate: (view: View) => void; surveyIdFilter?: string }) {
  ...
  const fieldAnalysis = useMemo(() => {
    ...
    for (const field of fields) {
      ...
      // Percentage = TIDAK/total (achievement rate): 100% TIDAK = no problems = Grade A
  ...
  const overallData = useMemo(() => {
    ...
    return {
      ...
      recommendation: getOverallRecommendation(overallPct),
    };
  }, [fieldAnalysis]);
  ...
  const qualifiesForCertificate = allFieldsCompleted && Object.values(fieldAnalysis).every((d) => d.percentage === 100);
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <div className="flex items-center gap-4 mt-2 text-sm">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-gray-600">IYA: {overallData.iya}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <div className="w-3 h-3 rounded-full bg-emerald-500" />
                          <span className="text-gray-600 font-semibold">TIDAK: {overallData.tidak}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-gray-50 rounded-xl p-4">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-gray-700 mb-1">Saran untuk Kamu</p>
                      ...
                      <Card className="h-full border-2 overflow-hidden">
                        ...
                        <CardContent className="p-4">
                          ...
                          <div className="bg-gray-50 rounded-lg p-3">
                            <p className="text-xs text-gray-500 font-medium mb-1">Saran</p>
                            <p className="text-xs text-gray-600 leading-relaxed line-clamp-3">{rec.saran}</p>
                          </div>
                        </CardContent>
                      </Card>
                    </motion.div>
              ...
              <table className="w-full text-xs border-collapse border border-gray-300">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border border-gray-300 px-2 py-1 text-center">Persentase</th>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex-1">
                            <div className="flex justify-between text-sm mb-1">
                              <span className="text-red-600 font-medium">IYA: {iyaCount}</span>
                  ...
                  <table className="w-full text-[10px] border-collapse border border-gray-300 mb-2">
                    ...
                    <tbody>
                      {response.answers.map((ans, aIdx) => (
                        <tr key={aIdx} className={ans.value === "IYA" ? "bg-red-50" : ""}>
