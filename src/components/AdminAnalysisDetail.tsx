...
import {
  ArrowLeft, Download, AlertCircle, Award, CheckCircle2,
} from "lucide-react";
...
import { useAuth, apiFetch, FIELD_CONFIG, getGrade, getSelectedAnalysisUserId, setSelectedAnalysisUserId, KopSurat, ProfilSiswa, handlePrintPDF, type View } from "@/lib/app-shared";
...
export default function AdminAnalysisDetail({ onNavigate, onSelectCertificateStudent }: { onNavigate: (view: View) => void; onSelectCertificateStudent?: (studentId: string) => void }) {
  const { user } = useAuth();
  ...
  const overallPieData = [
    { name: "IYA", value: totalIya, fill: "#f59e0b" },      // IYA = problem (amber)
  ...
  const overallRecommendation = getOverallRecommendation(overallPercentage);
  const studentKelas = (studentProfile.grade || 7) as GradeLevel;

  // Check if student qualifies for certificate: 100% TIDAK across ALL 4 fields
  const allFieldsCompleted = fields.every((f) => fieldAnalysis[f]);
  const allFieldsPerfect = allFieldsCompleted && fields.every((f) => (fieldAnalysis[f]?.percentage || 0) === 100);
  const qualifiesForCertificate = allFieldsPerfect && overallPercentage === 100;
  const selectedStudentId = getSelectedAnalysisUserId();
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6 print:hidden">
        ...
        <div className="ml-auto flex items-center gap-2">
          {qualifiesForCertificate && selectedStudentId && (
            <Button
              size="sm"
              onClick={() => {
                if (onSelectCertificateStudent) onSelectCertificateStudent(selectedStudentId);
                onNavigate("certificate");
              }}
              className="bg-amber-500 hover:bg-amber-600 text-white"
            >
              <Award className="h-4 w-4 mr-2" /> Sertifikat
            </Button>
          )}
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  <div className="flex-1 w-full">
                    ...
                    <div className="flex justify-center gap-6 mt-2">
                      <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-500" /><span className="text-sm">IYA: {totalIya}</span></div>
                      <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-500" /><span className="text-sm">TIDAK: {totalTidak}</span></div>
                    </div>
                  </div>
                  {/* Overall Percentage & Grade */}
                  <div className="flex flex-col items-center gap-3 min-w-[200px]">
                    <div className={`text-5xl font-bold ${overallGradeInfo.color}`}>{overallPercentage}%</div>
                    <Badge className={`${overallGradeInfo.bgColor} ${overallGradeInfo.color} text-base px-4 py-1`}>
                      Grade {overallGradeInfo.grade} — {overallGradeInfo.label}
                    </Badge>
                    <div className={`p-3 rounded-lg ${overallGradeInfo.bgColor} border`}> 
                      <p className={`text-sm font-medium ${overallGradeInfo.color}`}>Rata-rata seluruh bidang</p>
                    </div>
                  </div>
                </div>
              </CardContent>
                ...
                return (
                  <Card key={field} className={`border-2 ${g.bgColor}`}>
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-xs text-gray-500">TIDAK: {fa.tidak} / {fa.total}</p>
                      <Badge className={`${g.bgColor} ${g.color} mt-1`}>Grade {g.grade} — {g.label}</Badge>
                    </CardContent>
                  </Card>
                );
              })}
                    ...
                    return (
                      <div key={field} className={`p-4 rounded-lg border-2 ${gInfo.bgColor}`}>
                        ...
                        <div className="flex items-center gap-2 mb-2 text-xs text-gray-600">
                          <span>Persentase: <strong>{pct}%</strong></span>
                          <span>•</span>
          ...
          <TabsContent value="per-bidang" className="mt-4">
            <div className="space-y-6">
              {fields.map((field) => {
                ...
                const rec = getFieldRecommendation(field, studentKelas, pct);
                const fieldPieData = [
                  { name: "IYA", value: fa.iya || 0, fill: "#f59e0b" },      // IYA = problem (amber)
                  { name: "TIDAK", value: fa.tidak || 0, fill: "#10b981" },   // TIDAK = no problem (emerald)
                ];

                const problems: any[] = fa.problems || [];

                // Risk level based on IYA percentage (higher IYA% = more problems = higher risk)
                const iyaPct = fa.total > 0 ? Math.round(((fa.iya || 0) / fa.total) * 100) : 0;
                let riskLevel = "Rendah";
                let riskColor = "text-emerald-600";
                let riskBg = "bg-emerald-50";
                if (iyaPct >= 51) { riskLevel = "Berat"; riskColor = "text-rose-600"; riskBg = "bg-rose-50"; }
                else if (iyaPct >= 26) { riskLevel = "Signifikan"; riskColor = "text-orange-600"; riskBg = "bg-orange-50"; }
                else if (iyaPct >= 11) { riskLevel = "Sedang"; riskColor = "text-amber-600"; riskBg = "bg-amber-50"; }
                else if (iyaPct >= 1) { riskLevel = "Ringan"; riskColor = "text-teal-600"; riskBg = "bg-teal-50"; }
                  ...
                  <Card key={field} className={`border-2 ${cfg.bgColor}`}>
                    <CardHeader>
                      <CardTitle className="text-base flex items-center gap-2">
                        <span className={cfg.color}>{cfg.icon}</span>
                        {cfg.label}
                        <Badge className={`${gInfo.bgColor} ${gInfo.color} ml-2`}>Grade {gInfo.grade} — {gInfo.label}</Badge>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <div className={`text-center mt-2 p-2 rounded-lg ${riskBg}`}>
                            <p className={`font-semibold ${riskColor}`}>Tingkat Masalah: {riskLevel} ({iyaPct}% IYA)</p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                              ...
                              <Badge className="bg-red-100 text-red-700 border-red-300 hover:bg-red-100 text-xs">{problems.length} pernyataan</Badge>
                            </h4>
                            {problems.length > 0 ? (
                              <div className="border rounded-xl overflow-hidden">
                                <div className="bg-amber-50 px-3 py-1.5 border-b flex items-center gap-2">
                                  <span className="text-[10px] font-semibold text-amber-800 w-6 text-center">No</span>
                                  <span className="text-[10px] font-semibold text-amber-800 flex-1">Pernyataan Masalah</span>
                                </div>
                                <div className="min-w-0">
                                  {problems.map((p: any, idx: number) => (
                                    <div key={idx} className={`flex items-start gap-2 px-3 py-2 border-b last:border-b-0 min-w-0 ${idx % 2 === 0 ? 'bg-white' : 'bg-amber-50/30'}`}>
                                      <span className="text-xs font-medium text-gray-500 w-6 text-center shrink-0 mt-0.5">{idx + 1}.</span>
                                      <p className="text-sm text-gray-800 leading-relaxed min-w-0 break-words">{p.text || p}</p>
                                    </div>
                                  ))}
                                </div>
                              </div>
                              ...
                              <div className="text-center p-4 text-gray-500 text-sm border rounded-xl bg-emerald-50/50">
                                ...
                                Tidak ada masalah teridentifikasi — Sangat Baik!
                              </div>
                            )}
                          </div>
                        </div>
