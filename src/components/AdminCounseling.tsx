...
import {
  ...
  KopSurat, ProfilSiswa, handlePrintPDF,
} from "@/lib/app-shared";
...
export default function AdminCounseling({ onNavigate }: { onNavigate: (view: View) => void }) {
  ...
  const [aiConfigStatus, setAiConfigStatus] = useState<"unknown" | "ok" | "not_configured" | "checking">("unknown");
  ...
  // TIDAK-based: higher TIDAK% = more problems = worse grade
  const tidakCount = formData.topicItems.filter(i => i.answer === "TIDAK").length;
  const iyaCount = formData.topicItems.filter(i => i.answer === "IYA").length;
  ...
  const checkAiConfig = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        setAiConfigStatus("ok");
        setAiConfigError("");
      } else {
        setAiConfigStatus("not_configured");
        setAiConfigError(result.message || "Gagal terhubung");
      }
    } catch (err) {
      setAiConfigStatus("not_configured");
      setAiConfigError(err instanceof Error ? err.message : "Gagal terhubung ke AI");
    }
  }, [user]);
  ...
  const handleGenerateAllAI = async () => {
    ...
    try {
      ...
      // Show a more helpful toast that guides the user to settings
      toast({
  ...
  const openAddDialog = () => {
    ...
    checkAiConfig(); // Check AI config when dialog opens
    setShowDialog(true);
  };
  ...
  const dibatalkan = counselings.filter(c => c.status === "DIBATALKAN").length;

  // Get student info for kartu
  const kartuStudent = kartuCounseling?.student
    ? { name: kartuCounseling.student.name, email: kartuCounseling.student.email, grade: kartuCounseling.student.grade, whatsapp: kartuCounseling.student.whatsapp, jenisKelamin: kartuCounseling.student.jenisKelamin || null, createdAt: kartuCounseling.student.createdAt || undefined }
    : null;
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card className="border-2 border-rose-100">
          <CardContent className="p-4 text-center">
            <XCircle className="h-6 w-6 mx-auto mb-1 text-rose-500" />
            <p className="text-2xl font-bold text-rose-700">{dibatalkan}</p>
                        ...
                        {cItems.length > 0 && (
                          <div className="mt-2 flex items-center gap-2">
                            <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full transition-all ${cTidakPct === 0 ? "bg-emerald-500" : cTidakPct <= 10 ? "bg-teal-500" : cTidakPct <= 25 ? "bg-amber-500" : cTidakPct <= 50 ? "bg-orange-500" : "bg-rose-500"}`} style={{ width: `${cTidakPct}%` }} />
                            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.status} onValueChange={(val) => {
                ...
                <SelectContent>
                  <SelectItem value="TERJADWAL">Terjadwal</SelectItem>
                  <SelectItem value="SELESAI">Selesai</SelectItem>
                  {editId && <SelectItem value="DIBATALKAN">Dibatalkan</SelectItem>}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-3">
              ...
              {formData.topicItems.length > 0 && (
                <div className="space-y-2">
                  {/* Percentage bar — TIDAK-based: higher TIDAK% = more problems = worse grade */}
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-2.5 bg-gray-200 rounded-full overflow-hidden">
                      <motion.div
                        className={`h-full rounded-full ${tidakPercentage === 0 ? "bg-emerald-500" : tidakPercentage <= 10 ? "bg-teal-500" : tidakPercentage <= 25 ? "bg-amber-500" : tidakPercentage <= 50 ? "bg-orange-500" : "bg-rose-500"}`}
                        initial={{ width: 0 }}
            ...
            <div className="space-y-2">
              ...
              {aiConfigStatus === "not_configured" && (
                <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 text-amber-600 mt-0.5 shrink-0" />
                  <div className="flex-1">
                    <p className="text-xs font-semibold text-amber-800">AI Belum Terhubung</p>
                    {aiConfigError ? (
                      ...
                      <p className="text-xs text-amber-700 mt-0.5">
                        Konfigurasi AI belum tersedia atau gagal terhubung.
                      </p>
                    )}
                    <div className="flex items-center gap-2 mt-1.5">
                      <button
                        type="button"
                        className="text-xs underline font-semibold text-amber-800 hover:text-amber-900"
              ...
              {aiConfigStatus === "checking" && (
                <div className="p-2 rounded-lg bg-gray-50 border border-gray-200 flex items-center gap-2">
                  <RefreshCw className="h-3 w-3 animate-spin text-gray-400" />
                  <p className="text-xs text-gray-500">Memeriksa koneksi AI...</p>
                </div>
              )}
              {aiConfigStatus === "ok" && (
                <div className="p-2 rounded-lg bg-emerald-50 border border-emerald-200 flex items-center gap-2">
                  <CheckCircle2 className="h-3 w-3 text-emerald-600" />
                  <p className="text-xs text-emerald-700 font-medium">AI Terhubung — siap generate</p>
                </div>
              )}
              <Button
                type="button"
            ...
            {kartuCounseling && (
              <div className="mt-4">
                ...
                {(() => {
                  ...
                  return (
                    <div className="mt-4">
                      <p className="text-sm font-semibold text-gray-700 mb-2 border-b pb-1">Pernyataan Masalah:</p>
                      <table className="w-full text-sm border-collapse">
                        <thead>
                          <tr className="bg-gray-100">
                            <th className="border border-gray-300 px-2 py-1.5 w-10 text-center">No</th>
                            <th className="border border-gray-300 px-2 py-1.5 text-left">Pernyataan</th>
                            <th className="border border-gray-300 px-2 py-1.5 w-20 text-center">Bidang</th>
                            <th className="border border-gray-300 px-2 py-1.5 w-20 text-center">Jawaban</th>
                          </tr>
                        </thead>
                        <tbody>
                          {items.map((item, idx) => (
                            <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                              ...
                              <td className="border border-gray-300 px-2 py-1.5">{item.text}</td>
                              <td className="border border-gray-300 px-2 py-1.5 text-center text-xs">{item.field || "-"}</td>
                              <td className={`border border-gray-300 px-2 py-1.5 text-center font-medium ${item.answer === "IYA" ? "text-amber-700" : "text-emerald-700"}`}>
                                {item.answer}
                              </td>
                            </tr>
                          ))}
                      ...
                      <div className="mt-2 flex gap-4 text-xs text-gray-500">
                        <span>IYA (Bermasalah): {items.filter(i => i.answer === "IYA").length}</span>
                        <span>TIDAK (Tidak Bermasalah): {items.filter(i => i.answer === "TIDAK").length}</span>
                      </div>
                    </div>
                  );
                })()}
                ...
                {(kartuCounseling.ringkasan || kartuCounseling.notes || kartuCounseling.followUp || kartuCounseling.solusi) && (
                  <div className="mt-4 space-y-3">
                    {kartuCounseling.ringkasan && (
                      <div>
                        <p className="text-sm font-semibold text-gray-700 border-b pb-1 mb-1">Ringkasan:</p>
                        <p className="text-sm text-gray-700 whitespace-pre-wrap p-2 bg-gray-50 rounded border border-gray-200">{kartuCounseling.ringkasan}</p>
                      </div>
                    )}
                    {kartuCounseling.notes && (
                      <div>
                        <p className="text-sm font-semibold text-gray-700 border-b pb-1 mb-1">Catatan:</p>
                        <p className="text-sm text-gray-700 whitespace-pre-wrap p-2 bg-gray-50 rounded border border-gray-200">{kartuCounseling.notes}</p>
                      </div>
                    )}
                    {kartuCounseling.followUp && (
                      <div>
                        <p className="text-sm font-semibold text-gray-700 border-b pb-1 mb-1">Tindak Lanjut:</p>
                        <p className="text-sm text-gray-700 whitespace-pre-wrap p-2 bg-blue-50 rounded border border-blue-200">{kartuCounseling.followUp}</p>
                      </div>
                    )}
                    {kartuCounseling.solusi && (
                      <div>
                        <p className="text-sm font-semibold text-gray-700 border-b pb-1 mb-1">Solusi:</p>
                        <p className="text-sm text-gray-700 whitespace-pre-wrap p-2 bg-emerald-50 rounded border border-emerald-200">{kartuCounseling.solusi}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="mt-8 pt-4 border-t">
              <div className="flex justify-between">
                <div className="text-center">
                  ...
                  <div className="w-32 h-16 border-b border-dashed border-gray-400 mt-1" />
                  <p className="text-xs font-semibold text-gray-800 mt-1">{schoolSettings.schoolPrincipal || "..........................."}</p>
                  {schoolSettings.schoolPrincipalNip && (
                    <p className="text-[10px] text-gray-500">NIP. {schoolSettings.schoolPrincipalNip}</p>
                  )}
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500">&nbsp;</p>
                  <p className="text-xs font-medium text-gray-700">Konselor Pendidikan</p>
                  <div className="w-32 h-16 border-b border-dashed border-gray-400 mt-1" />
                  <p className="text-xs font-semibold text-gray-800 mt-1">{schoolSettings.bkCoordinator || kartuCounseling?.bkOfficer || "..........................."}</p>
                  {schoolSettings.bkCoordinatorNip && (
                    <p className="text-[10px] text-gray-500">NIP. {schoolSettings.bkCoordinatorNip}</p>
                  )}
                </div>
              </div>
              <p className="text-center text-[9px] text-gray-400 mt-4 italic">
                Dicetak dari Sistem DCM Bimbingan Konseling — CekDiriBK.id
              </p>
            </div>
          </div>
