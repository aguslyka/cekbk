...
export async function GET(req: NextRequest) {
  try {
    ...
    let studentProfile: { name: string; email: string; grade: number; whatsapp: string | null; jenisKelamin: string | null; createdAt: string } | null = null;
    if (userId) {
      const user = await db.user.findUnique({
        where: { id: userId },
        select: { id: true, name: true, email: true, grade: true, whatsapp: true, jenisKelamin: true, createdAt: true },
      });
      if (user) {
        studentProfile = {
          name: user.name,
    ...
    interface FieldAnalysisEntry {
      ...
      percentage: number;
      problems: string[];
      users: { id: string; name: string; grade: number; whatsapp: string | null; iyaCount: number; tidakCount: number; total: number; percentage: number }[];
    }
    ...
    for (const fieldName of Object.keys(FIELD_NAMES)) {
      ...
      // If viewing specific user, collect their problem questions (IYA = has problem)
    ...
    const recommendationMatrix: Record<string, Record<number, { grade: string; percentage: number; tidakCount: number; total: number }>> = {};
