...
export async function PUT(
  ...
  try {
    ...
    // When status changed to SELESAI, clear topicItems in the database
    // (answers have already been updated above; items no longer needed)
    if (isStatusChangedToSelesai) {
      try {
        await db.counseling.update({
          where: { id },
          data: { topicItems: null },
        });
        counseling.topicItems = null;
      } catch (clearErr) {
        console.error('Failed to clear topicItems after SELESAI:', clearErr);
      }
    }
